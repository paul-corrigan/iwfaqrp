...
for ( tz in unique(all_monitors$meta$timezone) ) {
  ...
  dailyMeanList[[tz]] <-
    ...
    monitor_filterDate(20230415, 20231101) %>%
...
#pc: actually leave this next line commented out
...
p1 <- ggplot() +
  ...
  geom_point(data = df, mapping = aes(longitude, latitude, size = usg_bin), color = US_AQI$colors_EPA[3]) +
...
p2 <- ggplot() +
  ...
  geom_point(data = df, mapping = aes(longitude, latitude, size = unhealthy_bin), color = US_AQI$colors_EPA[4]) +
  scale_size(breaks = 1:(length(bin_thresholds)-1), labels = bin_labels, name = 'Number of days') +
  # theme_nothing(legend = TRUE) +
  coord_map()
...
p4 <- ggplot() +
  ...
  ylim(min(states_outline$lat),max(states_outline$lat)) +
  geom_polygon(data = states_outline, mapping = aes(long, lat, group = group), fill = NA, colour = "grey50") +
  geom_point(data = df, mapping = aes(longitude, latitude, size = hazardous_bin), color = US_AQI$colors_EPA[6]) +
  scale_size(breaks = 1:(length(bin_thresholds)-1), labels = bin_labels) +
  # theme_nothing() +
  coord_map()
...
ggsave(ggpubr::ggarrange(p1, p2, p3, p4, ncol=4, nrow=1, common.legend = TRUE, legend="bottom"),
       filename = '2023_whole_us_monitor_days.png', bg="white")
...
ggsave(p1, filename = '/Users/katecorrigan/Documents/R/IWFAQRP/Output/2023_USGmap.png')
ggsave(p2, filename = '/Users/katecorrigan/Documents/R/IWFAQRP/Output/2023_Unhealthymap.png')
ggsave(p3, filename = '/Users/katecorrigan/Documents/R/IWFAQRP/Output/2023_VeryUnhealthymap.png')
ggsave(p4, filename = '/Users/katecorrigan/Documents/R/IWFAQRP/Output/2023_Hazmap.png')
